...
overall_data=data[data['State']=='India'].iloc[16:,:].reset_index().drop('index',axis=1)
...
dates=list(set(overall_data['Date'].values))
...
for i in range(0,74):
    ...
    end_day=start_day+6
    week_count={}
    week_data=overall_data[overall_data['Date']==dates[end_day]]
    if subtract_date>=0:
      sub_data=overall_data[overall_data['Date']==dates[subtract_date]]
...
for i in range(0,17):
    ...
    month_count={}
    month_data=overall_data[overall_data['Date']==dates[end_day]]
    if subtract_date>=0:
      sub_data=overall_data[overall_data['Date']==dates[subtract_date]]
      old_state_ids=sub_data['State'].values
...
for d_id in ['India']:
    ...
    overall_dict.append((d_id,'week'+str(x1[i_w1]),'week'+str(x1[i_w2]),'month'+str(x2[i_m1]),'month'+str(x2[i_m2])))
...
overall_dict.insert(0,('Country','Wave 1 Week Id','Wave 2 Week Id','Wave 1 Month Id','Wave 2 Month Id'))


# In[17]:


file = open('../Generated/4/overall-peak.csv', 'w+', newline ='')
  
# writing the data into the file
with file:    
    write = csv.writer(file)
    write.writerows(overall_dict)
